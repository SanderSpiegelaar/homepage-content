...
Replace the pilot's Zod request and response schemas with `effect/Schema` decoding so validation failures participate in the typed Effect channel without adapters. Because these are the project's only remaining Zod schemas, remove the unused Zod dependency rather than retaining two validation libraries.
...
- **Effect Schema is less familiar to current contributors than Zod** → Keep schemas local to migrated boundaries and use the pilot as the concrete reference.
...
1. Add `effect`, remove the unused `zod` dependency after migrating its final schemas, and confirm the existing TypeScript configuration is supported.
